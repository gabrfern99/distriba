...
export function TenantEditForm({ tenant }: { tenant: Tenant }) {
  ...
  useEffect(() => {
    if (state?.success) toast('Tenant atualizado!')
    ...
    <form action={formAction} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <Input id="name" name="name" label="Nome *" defaultValue={tenant.name} required />
        </div>
        <Input id="slug" name="slug" label="Slug" defaultValue={tenant.slug} />
        ...
        <Input id="state" name="state" label="UF" maxLength={2} defaultValue={tenant.state ?? ''} />
